...
class Player(object):
    ...
    def RandomRoll(self):
        ...
        for i in range(random.randint(1,self.Strategies['Random'])):
            ...
            if roll == 0:
                break
            Score += roll
            if Score + self.TotalScore >= Game.TargetScore:
                print(self.Name + " has just reached " + str( Game.TargetScore) + " points and is stopping")
                break
    ...
    def Sprint_To_Finish(self):
        Score = 0
        NumRolls = 0
        for i in range(50): # rolls till 0 or finish
            NumRolls += 1
            roll = self.pig.Roll()
            if roll == 0:
                break
    ...
    def Aggresive(self):
        Score = 0
        NumRolls = 0
        for i in range(random.randint(6,8)): # Aggresive players roll between 6 and 8
            NumRolls += 1
            roll = self.pig.Roll()
            if roll == 0:
                break
    ...
    def Cautious(self): # Cautious players roll between 1 and 4
        Score = 0
        NumRolls = 0
...
class Game(object):
    ...
    def __init__(self, **kwargs):
        ...
        Game.TargetScore = kwargs['target_score']   # game winning score
    ...
    def WinnerExists(self):
        for name,PlayerObj in self.Players.items():
            if PlayerObj.TotalScore >= Game.TargetScore:
